# 吾郎の不思議なダンジョン

ブラウザで動く、シンプルなローグライク（不思議のダンジョン風）です。
https://thashimoto-visco.github.io/game_goro_dangeon/

## 遊び方

1. `index.html` をブラウザで開く。
2. タイトルで任意のキーを押して開始。`H` で操作説明、`S` で音のON/OFFを切り替えられる。
3. 矢印キー または `WASD` で移動。
4. 隣接する敵の方向へ移動入力すると通常攻撃（1ターン経過）。
5. `Space` で足踏み（1ターン経過）。
6. 床アイテムの上に移動すると自動で拾得。
7. `I` で持ち物メニューを開く。上下で選択、`Enter` で使用/装備、`T` で投げる、`D` で足元に置く、`Escape` で閉じる。
8. `1`-`9` で即使用・装備、`Shift`+番号で投げる、`Alt`+番号で足元に置く。`Shift`+`Alt`+番号も置く操作になる。
9. 青いマス（階段）に乗ると次の階へ。
10. HPが0になると全滅画面が表示される。`R` で即再挑戦、`Enter` で今回のリザルトを確認できる。
11. リザルトでは `R` で再挑戦、`Escape` でタイトルへ戻る。
12. `音 OFF` ボタンを押すと音が有効になり、もう一度押すとミュート。
13. 敵を倒すと経験値を得て、一定値でレベルアップ。
14. 満腹度が十分ある時は、ターン経過でHPが少しずつ自然回復。
15. 低確率で宝物部屋や回復の泉が出現する。
16. 回復の泉に乗ると、HPを回復し、満腹度を現在の最大値まで全回復できる。泉は1回だけ使える。
17. 一度探索した場所は薄暗い「記憶表示」で見え続ける。
18. `M` で画面右上のミニマップを表示/非表示。
19. `Shift` + 方向キーでダッシュ。一本道の曲がり角は自動で曲がる。敵が見えた時、分かれ道、入口、アイテム、階段などで自動停止。何かキーを押すと中断。
20. 5Fでは、低確率イベントの代わりに強敵のミニ悪魔ちゃんが現れることがある。撃破は必須ではない。
21. 武器と盾は別々に装備できる。盾は被ダメージを減らす。
22. 強化の巻物を読むと、装備中の武器（なければ盾）が `+1` される。上限は `+3`。
23. いかずちの巻物を読むと、同じ部屋の敵にまとめてダメージが入る。
24. 大きなおにぎりを食べると、満腹度の最大値が上がる（上限150）。
25. 悪魔斬りの剣は、悪魔タグを持つ敵に大きなダメージを与える。
26. 3F以降のどくトカゲは、攻撃時に30%で毒を与える。毒は8ターン、毎ターン1ダメージを与え、その間は自然回復を止める。
27. 4F以降のぷくぷくキノコは、直線4マス以内にいる吾郎へ胞子弾を撃ち、隣接されると距離を取る。
28. なおし草は状態異常をすべて治す。
29. 眠りの杖は、向いている方向の直線6マス以内にいる最初の敵を5ターン眠らせる。残り回数は表示名の `[3]` で確認でき、0回の杖を振ろうとしてもターンは進まない。
30. 3F以降では小部屋群、6F以降では大広間が抽選される。階段移動カードとログで今回のフロアテーマを確認できる。

## 仕様（現状）

- タイトル、操作説明、プレイ中、リザルトの4シーン
- 到達階層、撃破数、強敵撃破数、レベル、経験値から算出するスコア
- 項目ごとの自己ベストと冒険回数のローカル保存（保存不可環境ではゲーム進行を継続）
- 使用アイテム数、拾得数、通過フロアテーマを含む冒険結果
- 標準・大広間・小部屋群の3テーマを持つ、ランダム生成の部屋＋通路マップ
- テーマごとの部屋数、部屋サイズ、追加接続率、階段距離、敵数、アイテム数、暗闇色
- 画面より広いマップをプレイヤー追従カメラでスクロール表示
- 吾郎の方向表示と移動時の短い歩行モーション
- 隣接敵方向への移動入力で発生するターン制通常攻撃
- 攻撃モーション、斬撃、ヒット、敵リアクション、撃破演出
- 攻撃命中時のヒットストップ（撃破時は長め）と種別ごとのノックバック
- ダメージを受けた敵の頭上HPバー（残量で色が変化）
- 敵が視界に入った瞬間の「！」マーク、遭遇SE、出現ログ（視認中の敵は動きが速くなる）
- 低HP時に画面端が赤く脈動する警告ビネット
- 床アイテム、9枠の持ち物、武器/盾/食料/薬/巻物/杖の6種別
- 武器と盾の独立した装備スロット（同じものを選び直すと解除）
- 持ち物メニューと番号ショートカットで使用、装備、投げる、足元に置く操作
- 種別ラベル、強化後の実数値、フレーバー文を出す2行の説明欄
- 武器の攻撃補正と盾の守備補正を戦闘計算に反映
- 装備の強化値（`+1`〜`+3`）と、表示名/効果文への反映
- 装備を強化する巻物と、同室の敵をまとめて攻撃する巻物
- 持ち物を向いている方向へ投げる操作（最大6マス、命中で消費、外れると落下）
- 満腹度の最大値と、それを上げる食料
- モンスターのタグと、タグに対する特効を持つレア武器
- 毒と眠りを同じデータ形式で扱う状態異常システム
- プレイヤーの状態欄（残りターン付き）と、敵頭上の状態異常マーク
- 毒による継続ダメージ、自然回復停止、専用死因
- 敵にも共通形式で状態異常を付与できる仕組みと、stoneタグの毒耐性（現状、敵へ毒を与えるプレイ手段はない）
- ダメージで解除される眠りと、残り3回の眠りの杖
- 既存追跡AIと、射線・射程・退避判断を持つ遠距離AI
- 3F以降のどくトカゲと、4F以降のぷくぷくキノコ
- 敵ごとの経験値、プレイヤーレベル、経験値テーブル
- レベルアップ時の最大HP/攻撃/守備上昇とHP全回復
- レベル、経験値、次レベルまでのステータス表示
- 満腹度に応じたHP自然回復
- 階層別の敵出現テーブル（1Fはスライム中心、深い階ほどコウモリ/ゴーレムが増加）
- 階層別のアイテム出現テーブル（深い階ほど鉄の剣/鉄の盾、巻物、レア武器が出やすい）
- 既知領域の記憶表示（探索済みの地形を薄暗く表示。敵・アイテムは視界内のみ）
- 視界境界のソフト化（タイルごとの明度フェード、境界1マスの中間明度、吾郎周辺の淡い明かり）
- フロア深度による暗闇の色調変化（浅層は青系、4F〜は紫系、7F〜は赤黒系）
- 探索済み領域を映すミニマップ（プレイヤー位置、視認済み階段。`M` で切り替え）
- `Shift`+方向キーの通路ダッシュ（1歩ごとにターン消費。曲がり角は自動追従、敵視認・分かれ道・入口・アイテム・階段・イベント・被ダメージで自動停止し、理由が分かりにくい停止はログに表示）
- 階段移動時の遷移演出（暗転→フロア番号カード→フェードイン。遷移中は入力ロック）
- 階段移動カードとログに表示されるテーマ名・到着テキスト
- 低確率で出現する特殊部屋
- 宝物部屋の追加アイテム配置
- HPを回復し、満腹度を現在の最大値まで全回復する1回だけ使える泉
- 敵種とは独立した通常／強敵の遭遇ランク
- 5Fの検証用強敵遭遇（安全な特殊部屋を作れないフロアでは進行優先でスキップ）
- 強敵の階層予兆、部屋発見、初回視認ログと専用演出
- 強敵撃破時の経験値補正、確定アイテムドロップ、強敵撃破数
- 全滅時の死因表示、暗転、倒れ演出、戦績表示、再挑戦導線
- 初期ミュートのSE/BGMと音ON/OFFボタン
- 部屋と通路でピッチの違う足音、未探索部屋への初入室スティング
- 低HP時の心音とBGMの曲調変化、下降音形のフロア降下ジングル
- 満腹度システム（0になると毎ターンダメージ）
- SFC風のステータス表示とログ
- ゲーム内吾郎idle・歩行・4方向攻撃: `assets/actors/goro/runtime/` 以下のPrototype 3 lossless WebP

- 参照用ラスタ画像: `assets/goro_reference_1.svg`
- 参照用ラスタ画像（2枚目）: `assets/goro_reference_2.svg`

- 旧/参照用吾郎画像: `assets/goro.svg`, `assets/goro_player.svg`, `assets/goro_sprite_sheet.svg`
- モンスター画像: `assets/monster_slime.svg`, `assets/monster_bat.svg`, `assets/monster_golem.svg`
- ミニ悪魔ちゃん画像: `assets/monster_mini_devil.svg`
- タイトル／リザルト背景: `assets/ui/title_bg.webp`, `assets/ui/result_bg.webp`

- タイル画像: `assets/tiles/floor_01.svg`, `assets/tiles/floor_02.svg`, `assets/tiles/floor_03.svg`, `assets/tiles/wall_01.svg`, `assets/tiles/wall_02.svg`, `assets/tiles/wall_03.svg`, `assets/tiles/stairs_down.svg`
- 仮アイコン画像: `assets/icons/item_weapon.svg`, `assets/icons/item_weapon_rare.svg`, `assets/icons/item_shield.svg`, `assets/icons/item_food.svg`, `assets/icons/item_potion.svg`, `assets/icons/item_scroll.svg`

## プレイヤーアニメーションの追加方法

吾郎の画像パス、描画サイズ、motion値、方向別clipは `player-actor.js` に集約しています。`main.js` はclipのコマ数を前提にせず、各フレームの `duration` と現在の歩行位相から表示画像を選びます。

- 下・上: 2コマ × 180ms。
- 右・左: 4コマ × 90ms。
- 全方向: 同じ足が再接地するまで360ms。
- 1マス移動: 半周期の180ms相当だけ歩行位相を進める。
- 攻撃: 4方向各1コマ。フレーム固有の `drawW` / `drawH` で横パンチのリーチを保つ。

前後方向を将来4コマへ増やす場合は、`player-actor.js` の該当clipへpassingを追加し、4フレームのduration合計を360msに保ちます。`main.js` の分岐追加は不要です。

関連テスト: `node tests/player-actor-tests.js`

## アイテムの追加方法

アイテム定義は `item-catalog.js`、種別ごとの共通情報は `item-system.js`、出現配分は `dungeon-item-tables.js` にあります。

新しいアイテムを足す場合は、`item-catalog.js` へ1件追加し、`dungeon-item-tables.js` の出現表へ入れます。既存の6種別（`weapon` / `shield` / `food` / `potion` / `scroll` / `wand`）の共通表示は `item-system.js` で処理されます。

```js
{
  key: "steelSword",
  name: "鋼の剣",
  kind: "weapon",
  icon: "weapon",
  atk: 5,
  throwPower: 8,
  flavor: "ずしりと重い。",
}
```

新しい種別を足す場合は、`item-system.js` の `KIND_PROFILES` に操作ラベルと装備スロットを追加し、`main.js` の `useInventorySlot()` に使用処理を1つ足します。

テストは `node tests/item-system-tests.js` で、出現表の全 `type` がカタログに存在することまで確認します。

## 状態異常と敵AIの追加方法

状態異常の数値・表示・解除条件は `status-catalog.js`、付与・経過・解除判定は `status-system.js` にあります。新しい状態異常は、既存フラグで表現できる範囲ならカタログへ1件足せます。

敵の判断は `monster-ai.js` が `wait` / `move` / `melee` / `ranged` のintentを返し、`main.js` が実行します。既存intentで表現できるAIなら、`monster-ai.js` のbehavior追加とモンスター定義の `ai` 指定だけで追加できます。

関連テスト:

- `node tests/status-system-tests.js`
- `node tests/monster-ai-tests.js`
- `node tests/monster-system-tests.js`
- `node tests/item-system-tests.js`

## フロアテーマの追加方法

テーマ定義は `floor-theme-catalog.js`、階層ごとの配分は `dungeon-theme-tables.js` にあります。新しいテーマはカタログへ生成条件・配置係数・表示を追加し、テーマテーブルへ重みを登録します。`dungeon-layout.js` はテーマ名を参照せず、受け取った生成パラメータだけでレイアウトを作ります。

ローカル環境では `?debug=1&floor=8&theme=greatHall` のように指定すると、任意階層を指定テーマで開始できます。`theme` は `standard` / `greatHall` / `warren` を指定でき、未知のキーは標準テーマへ戻ります。公開ホストではデバッグ指定は無効です。

関連テスト:

- `node tests/floor-theme-tests.js`
- `node tests/structure-tests.js`

## 配信時のアセットパス

`main.js` は `window.GORO_DUNGEON_CONFIG.assetBaseUrl` を基準に画像パスを解決します。

未指定の場合は `document.baseURI` を基準にします。GitHub Pagesなどのproject siteでは、`index.html` 側で現在のHTML位置を基準に注入しているため、`/assets/...` ではなく `./assets/...` 相当で解決されます。

別の配信先でアセットだけCDNなどへ置く場合は、`main.js` より前に以下のように指定できます。

```html
<script>
  window.GORO_DUNGEON_CONFIG = {
    assetBaseUrl: "https://example.com/game_goro_dangeon/",
  };
</script>
```

## ベスト記録の保存

ベスト記録はブラウザの `localStorage` に保存します。`file:` から直接開いた場合、プライベートウィンドウ、容量超過などで保存できないことがあります。その場合もタイトル、プレイ、リザルト、再挑戦は通常どおり動き、リザルトの記録欄だけ保存不可表示になります。

保存データのバージョンや値が不正な場合は、安全のため初期値へ戻します。

関連テスト:

- `node tests/run-record-tests.js`
- `node tests/app-startup-tests.js`
